...
const VendorDashboard = ({ user }) => {
  ...
  const tier = latestScore ? getTier(latestScore.total_pct) : null;
  const pendingInvoices = invoices.filter(i => i.status === "pending" || i.status === "submitted");
  const paidInvoices = invoices.filter(i => i.status === "paid");
  const totalBusiness = awards.reduce((sum, a) => sum + (parseFloat(a.agreed_amount) || 0), 0);
  const completedJobs = awards.filter(a => a.status === "confirmed" || a.status === "po_created").length;
